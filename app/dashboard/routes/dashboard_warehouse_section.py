...
@router.post("/warehouse_section")
def dashboard_warehouse_section(filters: DashboardRequest):
    ...
    with engine.connect() as conn:
        ...
        out["charts"]["warehouse_performance"] = [dict(r._mapping) for r in rows]
        ...
        query = f"""
            ...
            """
        rows = conn.execute(text(query), params).fetchall()
        out["trend_line"]["warehouse_performance"] = [dict(r._mapping) for r in rows]
        ...
        query = f"""
            ...
            """
        rows = conn.execute(text(query), p_params).fetchall()
        out["trend_line"]["purchase_warehouse_performance"] = [dict(r._mapping) for r in rows]
        ...
        query = f"""
            ...
            """
        rows = conn.execute(text(query), r_params).fetchall()
        out["trend_line"]["return_warehouse_performance"] = [dict(r._mapping) for r in rows]
